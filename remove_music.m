...
load([file_path filename '\silence_removal_1000ms'],'J_start','J_stop')

[sig,fs]=audioread([file_path filename '.wav']);
classif = trmszc([file_path filename '.wav'],0,[0 (length(sig)/fs)]);
...
save([file_path filename '\music_removal_LL'],'LLsp','LLnsp','T')

% Continuity constraint
% A = load('G:\Parthe\Dropbox\Acads\DDP\Demo\silence_removal_1000ms','J_start','J_stop');
A = load([file_path filename '\silence_removal_1000ms'],'J_start','J_stop');
min_length = 100;
...
save([file_path filename '\music_removal_1000ms'],'J_start','J_stop')
